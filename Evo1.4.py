...
print("GLOBALQ FULLY LOADED: Starting Trials")
        ...
        while env.Score1 + env.Score2 < 13:
            ...
            while P2:
                ...
                if len(env.Hand2) == 0:
                    P2 = False
                    break
                action = Counter(env.Hand2).most_common(1)[0][0]
        ...
        if not env.Cards and not env.Hand1 and not env.Hand2:
            if env.Score1 < env.Score2:
                for i in range(len(states1)):
                    if env.Q[states1[i]][action1[i] - 1] > -20:
                        reward = -((env.Score2 + 5) / (env.Score1 + 5))
                        env.Q[states1[i]][action1[i] - 1] += alpha * (reward - env.Q[states1[i]][action1[i] - 1])
            else:
                for i in range(len(states1)):
                    if env.Q[states1[i]][action1[i] - 1] < 20:
                        reward = 2 + ((env.Score2 + 5) / (env.Score1 + 5))
                        env.Q[states1[i]][action1[i] - 1] += alpha * (reward - env.Q[states1[i]][action1[i] - 1])
    return dict(env.Q)
